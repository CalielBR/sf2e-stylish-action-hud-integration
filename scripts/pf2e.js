...
import { getIconHtml, getIconPath } from "./icon-config.js";
...
export class Pf2eAdapter extends BaseSystemAdapter {
	...
	getDefaultLayout() {
		return [
			{
				systemId: "strike",
				label: `${game.i18n.localize("IBHUD.Category.Strike")} <span class="material-symbols-outlined" style="margin-left: 15px;">swords</span>`, // Spacing for Strike button
				icon: "",
				type: "submenu",
				useSidebar: false,
			},
			{
				systemId: "spell",
				label: `${game.i18n.localize("IBHUD.Category.Spells")} <span class="material-symbols-outlined" style="margin-left: 15px;">auto_awesome</span>`,
				icon: "",
				type: "submenu",
				useSidebar: true,
			},
			{
				systemId: "action",
				label: `${game.i18n.localize("IBHUD.Category.Actions")} <span class="material-symbols-outlined" style="margin-left: 15px;">engineering</span>`,
				icon: "",
				type: "submenu",
				useSidebar: true,
			},
			{
				systemId: "feat",
				label: `${game.i18n.localize("IBHUD.Category.Feats")} <span class="material-symbols-outlined" style="margin-left: 15px;">military_tech</span>`,
				icon: "",
				type: "submenu",
				useSidebar: true,
			},
			{
				systemId: "utility",
				label: `${game.i18n.localize("IBHUD.Category.Utility")} <span class="material-symbols-outlined" style="margin-left: 15px;">build</span>`,
				icon: "",
				type: "submenu",
				useSidebar: true,
			},
			{
				systemId: "inventory",
				label: `${game.i18n.localize("IBHUD.Category.Inventory")} <span class="material-symbols-outlined" style="margin-left: 15px;">inventory_2</span>`,
				icon: "",
				type: "submenu",
				useSidebar: true,
			},
	...
	_getStrikeData(actor) {
		...
		// Inclui ataques válidos que não estejam guardados em containers (stowed)
		const strikes = allActions.filter((a) => a.item);

		const items = [];

		// [A] General Strikes
		strikes.forEach((s) => {
			const item = this._createStrikeItem(s);
	...
	_createStrikeItem(s) {
        ...
        // Get proficiency rank for the strike
        const rank = s.statistic?.rank ?? 0;
        const rankLetter = ["", "T", "E", "M", "L"][rank] || "";
        const rankLabel = rankLetter ? game.i18n.localize(`PF2E.ProficiencyLevel${rank}`) : "";
        const profBgColor = ["#6b7280", "#2c4e69", "#5b3a96", "#3a7b59", "#98503d"][rank] ?? "#6b7280";
        const profLabel = rankLabel || game.i18n.localize("PF2E.ProficiencyLevel0") || "Untrained";
        const profBadgeHtml = `
            <span style="
                ...
                box-shadow: 0 0 4px ${profBgColor};
            ">${profLabel}</span>`;
		...
		if (isReady) {
			...
			buttonsHtml += `
					...
					>
							${getIconHtml('btn-damage', { size: '12px' })}
					</button>
					<button
						type="button"
						class="pf2e-crit-btn"
						onclick="event.stopPropagation(); StylishAction.useItem('${s.item.id}_critical')"
						...
						onmouseout="this.style.background='rgba(100, 20, 20, 0.8)'; this.style.color='#faa';"
					>
							${getIconHtml('btn-damage-critical', { size: '12px' })}
					</button>
				</div>`;
		}

		// [A-2] altUsages strike - For off-hand attacks, secondary strike modes, or special conditions
		let altButtonsHtml = "";
		if (s.altUsages?.length > 0) {
			s.altUsages.forEach((alt, altIdx) => {
				...
				altButtonsHtml += `
					...
					<button type="button" class="pf2e-crit-btn"
						onclick="event.stopPropagation(); StylishAction.useItem('${s.item.id}_altcritical_${altIdx}')"
						title="Critical Damage"
						style="background: rgba(100, 20, 20, 0.8); border: 1px solid #d44; color: #faa; 
							border-radius: 3px; padding: 1px 6px; font-size: 0.85em; cursor: pointer;
							line-height: 1.2; margin-left: 2px;"
		...
		const rawUnequipped = game.i18n.localize("IBHUD.Pf2e.ActionUnequipped");
		const unequippedLabel = !isReady ? `<span style="font-size:0.8em; color:#aaa; font-style:italic; margin-left:5px;">(${rawUnequipped === "IBHUD.Pf2e.ActionUnequipped" ? "Unequipped" : rawUnequipped})</span>` : "";

		// Sanitize label if it contains residual technical keys from old localization files
		let cleanLabel = s.label || "Strike";
		if (cleanLabel.toLowerCase().includes("ibhud.")) cleanLabel = s.item?.name || s.slug?.replace(/-/g, ' ').capitalize() || "Strike";
		...
		return {
			...
			_isUnarmed: isUnarmed, // additional property to identify unarmed strikes for special handling if needed
		};
	}
	...
	_createElementalBlastItem(elementalBlast, config) {
		...
		buttonsHtml += `
			...
			<button type="button" class="pf2e-crit-btn"
				onclick="event.stopPropagation(); StylishAction.useItem('${blastId}_critical')"
				title="Critical Damage"
				style="background: rgba(100, 20, 20, 0.8); border: 1px solid #d44; color: #faa; 
					border-radius: 3px; padding: 1px 6px; font-size: 0.85em; cursor: pointer;
					line-height: 1.2; margin-left: 2px;"
	...
	_getStrikeButtonsForPopup(strike) {
		...
		html += `
            <button 
                ...
                "
            >
                ${getIconHtml('btn-damage', { size: '1.2em', extraStyle: 'vertical-align: middle;' })}
            </button>
            <button 
                type="button"
                class="pf2e-crit-btn"
                data-action="clickStrike" 
                data-command="critical"
                data-item-id="${strike.item.id}"
                title="Critical Damage"
                style="
                    background: rgba(150, 20, 20, 0.8); 
                ...
                "
            >
                ${getIconHtml('btn-damage-critical', { size: '1.2em', extraStyle: 'vertical-align: middle;' })}
            </button>
        </div>`;
	...
	_getActionData(actor) {
		...
		// [NEW] Skill Actions & Basic Actions
		const basicActions = [];
		const skillActions = [];
	...
	_getInventoryData(actor) {
		...
		const categories = {
			weapon: {
				label: `${game.i18n.localize("IBHUD.Pf2e.InvWeapons") || "Weapons"} ${getIconHtml('submenu-weapons', { margin: '0 0 0 12px' })}`, // Spacing for inventory categories
				tooltip: game.i18n.localize("IBHUD.Pf2e.InvWeapons"),
			},
			armor: {
				label: `${game.i18n.localize("IBHUD.Pf2e.InvArmor")} ${getIconHtml('submenu-armor', { margin: '0 0 0 12px' })}`,
				tooltip: game.i18n.localize("IBHUD.Pf2e.InvArmor"),
			},
			consumable: {
				label: `${game.i18n.localize("IBHUD.Pf2e.InvConsumables")} ${getIconHtml('consumables', { margin: '0 0 0 12px' })}`,
				tooltip: game.i18n.localize("IBHUD.Pf2e.InvConsumables"),
			},
			equipment: {
				label: `${game.i18n.localize("IBHUD.Pf2e.InvEquipment")} ${getIconHtml('equipment-tools', { margin: '0 0 0 12px' })}`,
				tooltip: game.i18n.localize("IBHUD.Pf2e.InvEquipment"),
			},
			treasure: {
				label: `${game.i18n.localize("IBHUD.Pf2e.InvTreasure")} ${getIconHtml('treasure-credits', { margin: '0 0 0 12px' })}`,
				tooltip: game.i18n.localize("IBHUD.Pf2e.InvTreasure"),
			},
			backpack: {
				label: `${game.i18n.localize("IBHUD.Pf2e.InvContainers")} ${getIconHtml('menu-inventory', { margin: '0 0 0 12px' })}`,
				tooltip: game.i18n.localize("IBHUD.Pf2e.InvContainers"),
			},
			ammo: {
				label: `Ammunition ${getIconHtml('ammunition', { margin: '0 0 0 12px' })}`,
				tooltip: "Ammunition",
			},
		};
	...
	_getUtilityData(actor) {
		...
		if (actor.saves) {
			["fortitude", "reflex", "will"].forEach((saveKey) => {
				...
				if (save) {
					...
					const rank = save.rank ?? 0;
					const rankLetter = ["", "T", "E", "M", "L"][rank] || "";
					const rankLabel = rankLetter ? game.i18n.localize(`PF2E.ProficiencyLevel${rank}`) : "";
					const circleColor = ["", "#2c4e69", "#5b3a96", "#3a7b59", "#98503d"][rank] || "#6b7280";
					const circleHtml = rankLetter ? `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background-color:${circleColor};margin-right:4px;vertical-align:middle;box-shadow:0 0 3px ${circleColor};"></span>` : "";
					const rankHtml = rankLetter ? `<span title="${rankLabel}" style="margin-right:4px;font-size:0.8em;opacity:0.9;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:3px;padding:1px 4px;display:inline-flex;align-items:center;">${circleHtml}${rankLetter}</span>` : "";
					const iconKey = `save-${saveKey}`;
					const iconHtml = getIconHtml(iconKey, { color: '#fff', margin: '0 12px 0 0' });

					const saveProfBadge = rankLetter
						? `<span style="display:inline-flex;align-items:center;gap:5px;margin-bottom:5px;font-size:0.85em;">
							${circleHtml}<span style="color:var(--sf2e-checks-color,#c084fc);font-weight:bold;">${rankLabel}</span>
						</span><br>`
						: "";
					items["save"]["all"].push({
						id: `save-${saveKey}`,
						...
						description: `${saveProfBadge}<div style="margin-bottom:5px;padding:2px 6px;background:rgba(167,139,250,0.1);border-left:3px solid var(--sf2e-checks-color, #c084fc);border-radius:2px;"><strong style="color: var(--sf2e-checks-color, #c084fc);">Modifier: ${mod >= 0 ? "+" : ""}${mod}</strong></div>Roll ${save.label} Save`,
					});
				}
			});
		}
		...
		if (actor.skills) {
			...
			skillKeys.forEach((key) => {
				...
				if (skill.label) {
					...
					const circleHtml = rankLetter ? `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background-color:${circleColor};margin-right:4px;vertical-align:middle;box-shadow:0 0 3px ${circleColor};"></span>` : "";
					const rankHtml = rankLetter ? `<span title="${rankLabel}" style="margin-right:4px;font-size:0.8em;opacity:0.9;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:3px;padding:1px 4px;display:inline-flex;align-items:center;">${circleHtml}${rankLetter}</span>` : "";
					const skillProfBadge = rankLetter
						? `<span style="display:inline-flex;align-items:center;gap:5px;margin-bottom:5px;font-size:0.85em;">
							${circleHtml}<span style="color:var(--sf2e-checks-color,#c084fc);font-weight:bold;">${rankLabel}</span>
						</span><br>`
						: "";
					items["skill"]["all"].push({
						...
						cost: `<span style="color: var(--sf2e-checks-color, #c084fc); font-weight: bold; display: flex; align-items: center;">${rankHtml}${mod >= 0 ? "+" : ""}${mod}</span>`,
						description: `${skillProfBadge}<div style="margin-bottom:5px;padding:2px 6px;background:rgba(167,139,250,0.1);border-left:3px solid var(--sf2e-checks-color, #c084fc);border-radius:2px;"><strong style="color: var(--sf2e-checks-color, #c084fc);">Modifier: ${mod >= 0 ? "+" : ""}${mod}</strong></div>Roll ${skill.label}`,
					});
				}
			});
		}
		...
		if (percStat) {
			...
			const rank = percStat.rank ?? 0;
			const rankLetter = ["", "T", "E", "M", "L"][rank] || "";
			const rankLabel = rankLetter ? game.i18n.localize(`PF2E.ProficiencyLevel${rank}`) : "";
			const circleColor = ["", "#2c4e69", "#5b3a96", "#3a7b59", "#98503d"][rank] || "#6b7280";
			const circleHtml = rankLetter ? `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background-color:${circleColor};margin-right:4px;vertical-align:middle;box-shadow:0 0 3px ${circleColor};"></span>` : "";
			const rankHtml = rankLetter ? `<span title="${rankLabel}" style="margin-right:4px;font-size:0.8em;opacity:0.9;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:3px;padding:1px 4px;display:inline-flex;align-items:center;">${circleHtml}${rankLetter}</span>` : "";
			const percProfBadge = rankLetter
				? `<span style="display:inline-flex;align-items:center;gap:5px;margin-bottom:5px;font-size:0.85em;">
					${circleHtml}<span style="color:var(--sf2e-checks-color,#c084fc);font-weight:bold;">${rankLabel}</span>
				</span><br>`
				: "";
			items["skill"]["all"].unshift({
				...
				cost: `<span style="color: var(--sf2e-checks-color, #c084fc); font-weight: bold; display: flex; align-items: center;">${rankHtml}${mod >= 0 ? "+" : ""}${mod}</span>`,
				description: `${percProfBadge}<div style="margin-bottom:5px;padding:2px 6px;background:rgba(167,139,250,0.1);border-left:3px solid var(--sf2e-checks-color, #c084fc);border-radius:2px;"><strong style="color: var(--sf2e-checks-color, #c084fc);">Modifier: ${mod >= 0 ? "+" : ""}${mod}</strong></div>Roll Perception`,
			});
		}
		...
		if (init) {
			...
			const rank = init.statistic?.rank ?? 0;
			const rankLetter = ["", "T", "E", "M", "L"][rank] || "";
			const rankLabel = rankLetter ? game.i18n.localize(`PF2E.ProficiencyLevel${rank}`) : "";
			const circleColor = ["", "#2c4e69", "#5b3a96", "#3a7b59", "#98503d"][rank] || "#6b7280";
			const circleHtml = rankLetter ? `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background-color:${circleColor};margin-right:4px;vertical-align:middle;box-shadow:0 0 3px ${circleColor};"></span>` : "";
			...
			const initProfBadge = rankLetter
				? `<span style="display:inline-flex;align-items:center;gap:5px;margin-bottom:5px;font-size:0.85em;">
					${circleHtml}<span style="color:var(--sf2e-checks-color,#c084fc);font-weight:bold;">${rankLabel}</span>
				</span><br>`
				: "";
			items["other"]["all"].push({
				...
				cost: `<span style="color: var(--sf2e-checks-color, #c084fc); font-weight: bold; display: flex; align-items: center;">${rankHtml}${mod >= 0 ? "+" : ""}${mod}</span>`,
				description: `${initProfBadge}<div style="margin-bottom:5px;padding:2px 6px;background:rgba(167,139,250,0.1);border-left:3px solid var(--sf2e-checks-color, #c084fc);border-radius:2px;"><strong style="color: var(--sf2e-checks-color, #c084fc);">Modifier: ${mod >= 0 ? "+" : ""}${mod}</strong></div>Roll Initiative`,
			});
		}
	...
	_prepareUtilitySubTabLabels() {
		return {
			...
			skill: { all: `Skills & Lore ${getIconHtml('submenu-skills', { margin: '0 0 0 12px' })}` },
			skill: { all: `Skills ${getIconHtml('submenu-skills', { margin: '0 0 0 12px' })}` },
			toggle: { all: `Roll Options ${getIconHtml('submenu-triggers', { margin: '0 0 0 12px' })}` },
			other: { all: `Misc & Rest ${getIconHtml('submenu-rest', { margin: '0 0 0 12px' })}` },
	...
	async useItem(actor, itemId, event = null) {
		...
		debugLog(`Using item: ${itemId} for actor: ${actor.name}`);
		...
		if (strike) {
			if (command === "altattack" || command === "altdamage" || command === "altcritical") {
				const altIdx = variantIndex;
				const subIdx = parts.length > 3 ? parseInt(parts[3]) : 0;
				const altUsage = strike.altUsages?.[altIdx];
				if (altUsage) {
					let action = "attack";
					if (command === "altdamage") action = "damage";
					if (command === "altcritical") action = "critical";
					return this._handleStrike(altUsage, action, subIdx, cleanEvent);
				}
			}
	...
	async _handleStrike(strike, command, variantIndex, event) {
		...
		if (command === "damage") {
			return strike.damage({ event });
		} else if (command === "critical") {
			return strike.critical({ event });
		...
		new DialogV2({
			...
			actions: {
				clickStrike: (event, target) => {
					...
					if (cmd === "damage") {
						strike.damage({ event: cEvt });
					} else if (cmd === "critical") {
						strike.critical({ event: cEvt });
					} else if (cmd === "attack") {
